...
let DynamicForm = () => {
    ...
    const recommendedByAi = async () => {
        ...
        try {
            const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${import.meta.env.VITE_APP_GRAQ_API_KEY}`,
    ...
    return (
        <>
            ...
            {showModal && <Modal setFormInput={setFormInput} setShowModal={setShowModal} aiFields={suggestion} />}
        ...
        <>
            <div className="card">
                <div className="card-body">
                    <p>Recommended by AI</p>
                    {
                        aiFields.length === 0 ? <Loader /> :
                            aiFields?.map((item) => (<button className="btn btn-outline-success mx-2 my-2" onClick={() => setByAi(item.label, item.type, item.placeholder)}>{item.label}</button>))}

                    {/* <button className="btn btn-outline-success mb-2 mx-2" onClick={() => setByAi("item.label", "item.type", "item.placeholder")}>{"item.label"}</button>
                    <button className="btn btn-outline-success mb-2" onClick={() => setByAi("item.label", "item.type", "item.placeholder")}>{"item.label"}</button> */}

                    <p className="custom-msg"><span></span><b>Or add custom field</b> </p>
